...
const createUser = (req, res, next) => {
  upload(req, res, async (ree) => {
    const userSchema = joi.object({
      ...
      middleName: joi.string().min(3).max(16).required(),
      ...
      schoolName: joi.string().min(10).max(50).required(),
      collageName: joi.string().min(10).max(50).required(),
      about: joi.string().min(3).required(),
      hobby: joi.string().min(3).max(16).required(),
      file: joi.optional(),
      education: joi.string().min(2).max(3).required(),
